// gpu_util.cuh —— GPU 侧公共设施：错误检查宏、事件计时、公共 kernel
// 所有 .cuh 共用这里的 kernel，避免重复定义（main.cu 单 TU 一次编译）。
#pragma once
...
// 每 block 处理一行(长度 N)，选最小的 topk 个下标（可选写距离）；blockDim.x 为 2 的幂
// out_dist 传 nullptr 则只写下标。迭代式选择：每轮归约取全局最小并置 FLT_MAX。
