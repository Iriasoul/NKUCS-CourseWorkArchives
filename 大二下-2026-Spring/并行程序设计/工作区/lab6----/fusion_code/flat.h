// flat.h —— strategy 0：本机暴力检索基线（定分母）
// 环境变了，绝对延迟必与服务器不同 → 本机重测暴力基线，后续所有融合加速比以此为分母。
// 对照两档：scalar（SIMD 关）与 avx2（SIMD 默认底座），量化 SIMD 本身的加速比。
#pragma once
#include "bench_common.h"

// 暴力扫全库，dis = 1 - ip，保留 top-k
...
inline void run_flat(const float* base, size_t N, const float* query, size_t M,
    ...
    std::printf("\n== [S0] Flat 暴力基线 ==  N=%zu d=%zu  测试 query=%zu  k=%zu  AVX2=%d\n",
    ...
    std::printf("\n>>> 本机暴力基线延迟 = %.1f us/query (avx2)  <<< 后续加速比分母\n",
                ms_avx2 / M * 1000.0);
    std::printf("====================\n");
}
