// bench_common.h —— 期末融合实验公共测试底座（跨平台 / x86 AVX2）
// 口径与 lab04/lab05 完全一致：LoadData(fbin) + recall@k + 微秒计时 + Top-k 堆
// 说明：平时 SIMD 为 ARM-NEON，此处等价重写为 AVX2；SIMD 内积为 CPU 侧默认底座。
#pragma once
...
// 本机暴力基线（avx2 flat, 见 strategy 0），后续所有加速比以此为分母
static const double BENCH_BASELINE_US = 1562.5;

// ------------------------------------------------------------------ 读取 fbin
// 头 8 字节 = (n,d) int32，随后 n*d 个 T。
// 注意 GT 文件物理布局为 [头][n*d int32 ids][n*d float dists]，
// 以 T=int 读取恰好只取 ids 块（与 lab04/lab05 一致）。
template <typename T>
...
#if BENCH_HAVE_AVX2
// AVX2 + FMA：一次 8 路，dim=96 → 12 次；尾部标量收尾
...
// ------------------------------------------------------------------ 召回率
// results[i] = 第 i 条 query 返回的 top-k id 列表；gt 为 int ids 块，gt_d=100。
