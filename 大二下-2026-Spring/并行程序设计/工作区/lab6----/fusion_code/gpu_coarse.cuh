// gpu_coarse.cuh —— strategy 2：GPU 粗排器（cuBLAS GEMM + 设备侧 Top-k），融合一的 L0
// 建立 GPU-only 精确基线，并验证"每 query 取 rerank_k 候选"的能力供级联复用。
// 批量化：每批 batch_B 条 query 一次 GEMM，显存 O(batch_B * N)。
#pragma once
