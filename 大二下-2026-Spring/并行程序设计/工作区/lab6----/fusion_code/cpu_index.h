// cpu_index.h —— CPU 侧 IVF / IVF-PQ 索引（lab04 NEON → AVX2 移植 + OpenMP）
// 保留 lab04 的关键设计：内存重排(同簇连续)、SoA PQ 码、跨 centroid SIMD 建 LUT、
// 线程内局部无锁粗排+精排后归并。作融合一的 L1(IVF-PQ 查表) / L2(SIMD 精排)。
#pragma once
...
// IVF-SIMD 单查询串行版（供上层按 query 并行调用，避免嵌套 OMP）
...
// IVF-PQ + OpenMP：LUT → 簇级动态调度 PQ-ADC 局部粗排 → 线程内 SIMD 精排 → 归并
inline std::vector<uint32_t>
ivf_pq_search_omp(const float* q, size_t dim, size_t k, int nprobe, size_t rerank_k, int nthr) {
    ...
    #pragma omp parallel num_threads(nthr)
    {
        ...
        MaxHeap fine;     // 局部精排（原始 float SIMD）
        while (!coarse.empty()) {
...
// 供【融合一·级联】的 L2(CPU 精排) 使用：GPU 回传每条 query 的 rerank_k 候选 → CPU 重算精确距离
...
// 供【融合一·分流】的 CPU 分支使用：与 GPU 分支并行处理不相交的 query 区段
