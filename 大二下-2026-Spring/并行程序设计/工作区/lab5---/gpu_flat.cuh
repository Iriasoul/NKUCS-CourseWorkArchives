#pragma once
// =====================================================================
//  gpu_flat.cuh —— 暴力(brute-force)批次检索
//   - flat_search_cpu : CPU 基线 (OpenMP), 加速比参考
//   - FlatCtx         : 持久化上下文; base/query 一次上传, 只对查询计算计时
//   - flat_search_gpu_cpu : GEMM + CPU 选 top-k   (GPU+CPU)
//   - flat_search_gpu_gpu : GEMM + GPU 选 top-k   (GPU+GPU)
//  计时口径与 IVF 一致: 数据上传/显存分配为一次性开销, 不计入查询耗时。
// =====================================================================
...
// 持久化上下文: base/query/显存/句柄一次性准备好 (不计入查询耗时)
struct FlatCtx {
