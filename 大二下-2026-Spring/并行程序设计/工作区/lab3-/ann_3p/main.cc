...
//   0  = Flat-SIMD baseline        (Lab2 等价, 单线程多线程加速比的起点)
...
int main(int argc, char* argv[]) {
    ...
    // ===== 在此切换策略 =====
    int strategy     = 15;    // 0..18
    int rerank_k     = 500;   // PQ rerank 用 (策略 3-6, 10-15)
    int nprobe       = 16;    // IVF 用 (策略 7-15)
    int nlist        = 256;   // IVF 簇数

    // ===== 在线查询的并行配置 (与离线完全独立) =====
    int online_omp_threads = 4;  // 在线 OpenMP 策略 (1,5,8,11,14,17) 使用
    int num_pthreads       = 4;  // 在线 Pthread  策略 (2,6,9,12,15,18) 使用
    ...
    for (size_t i = 0; i < test_number; ++i) {
        ...
        switch (strategy) {
            ...
            case 15: res = ivf_pq2_search_pthread (q, vecdim, k, nprobe, rerank_k, num_pthreads); break;
            case 16: res = hnsw_search_baseline   (q, vecdim, k); break;
            case 17: res = hnsw_search_omp        (q, vecdim, k, online_omp_threads); break;
            case 18: res = hnsw_search_pthread    (q, vecdim, k, num_pthreads); break;
            default:
