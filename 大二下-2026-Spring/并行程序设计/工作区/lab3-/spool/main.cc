...
#include "thread_pool.h"   // 持久化 Pthread 线程池 (消除 per-query create/join 开销)
...
int main(int argc, char* argv[]) {
    ...
    int rerank_k     = 125;   // PQ rerank 用 (策略 3-6, 10-15)
    ...
    // ====== 在线搜索 ======
    // 线程池: 在查询循环外一次性创建, 消除 per-query pthread_create/join 开销
    // (OpenMP 内部已有等效线程池; Pthread 策略之前每次 query 创建/销毁, 是负优化的根因)
    ...
    // 销毁线程池 (必须在 gettimeofday 测量结束后, 在统计之前)
