...
// --------------------------------------------------------------------------
// 策略 17: HNSW 多入口 + OpenMP
// 入口选取: 把 [0, N) 均匀分 T 段, 每线程取自己段的中间节点
// 每线程维护私有 visited/候选集 → 无锁
// --------------------------------------------------------------------------
inline std::priority_queue<std::pair<float, uint32_t>>
                    ...
                    int num_threads = 4)
{
    ...
    std::vector<HNSWPthreadArgs> args(num_threads);
    std::vector<std::priority_queue<std::pair<float, uint32_t>>> local_res(num_threads);

    for (int t = 0; t < num_threads; ++t) {
        size_t seg = N / (size_t)num_threads;
        hnsw_tableint ep = (hnsw_tableint)((size_t)t * seg + seg / 2);
        if (ep >= (hnsw_tableint)N) ep = 0;
        args[t].query       = query;
        args[t].entry_point = ep;
        args[t].ef          = ef;
        args[t].k           = k;
        args[t].dist_fn     = dist_fn;
        args[t].dist_param  = dist_param;
        args[t].result      = &local_res[t];
        pthread_create(&tids[t], nullptr, hnsw_pthread_worker, &args[t]);
    }
