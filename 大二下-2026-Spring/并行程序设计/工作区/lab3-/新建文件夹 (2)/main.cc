// ===========================================================================
// 并行程序设计 Lab 3 - Pthread/OpenMP 编程 (ANN 选题)
// 在 Lab2 SIMD 实验基础上, 引入多线程并行优化.
//
// 策略列表 (通过 main 中的 strategy 变量切换):
//   1  = Flat-SIMD + OpenMP        (base 分块并行)
//   2  = Flat-SIMD + Pthread       (手动线程, 用于对比)
//   3  = PQ-Rerank-SIMD baseline   (Lab2 策略4 风格: 逐 centroid + AoS, 单线程)
...
//   10 = IVF-PQ-SIMD baseline      (单线程)
//   11 = IVF-PQ-SIMD + OpenMP      (完整多线程流水线)
//   12 = IVF-PQ-SIMD + Pthread     (完整 pthread 流水线)
// ===========================================================================
...
// Lab3 各策略模块
#include "flat_thread.h"   // 策略 1, 2
#include "pq_thread.h"     // 策略 3
#include "ivf.h"           // 策略 4, 5
#include "ivf_pq.h"        // 策略 6, 7
...
int main(int argc, char* argv[]) {
    // ===== 全局并行配置 =====
    // 离线建索引: 统一固定为 4 线程, 跑所有策略时索引构建条件一致, 不参与对比讨论
    ...
    // ===== 在线查询的并行配置 (与离线完全独立) =====
    int online_omp_threads = 4;  // 在线 OpenMP 策略 (1, 5, 8, 11) 使用
    int num_pthreads       = 4;  // 在线 Pthread 策略 (2, 6, 9, 12) 使用

    // 输出策略 + 在线查询使用的并行方式
    // (注: 所有策略的离线建索引阶段统一使用 OpenMP, 仅在线查询方式不同)
    bool uses_omp     = (strategy == 1 || strategy == 5 || strategy == 8 || strategy == 11);
    bool uses_pthread = (strategy == 2 || strategy == 6 || strategy == 9 || strategy == 12);
    bool is_baseline  = (strategy == 3 || strategy == 4 || strategy == 7 || strategy == 10);
    ...
    if (strategy >= 10 && strategy <= 12) {
        build_ivf(base, base_number, vecdim, nlist);
        build_ivf_pq(base_number, vecdim, 8);  // PQ_M = 8
    }
    ...
    for (size_t i = 0; i < test_number; ++i) {
        ...
        switch (strategy) {
            ...
            case 2:  res = flat_search_pthread    (base, q, base_number, vecdim, k, num_pthreads); break;
            ...
            case 6:  res = pq_search_pthread      (q, base, base_number, vecdim, k, rerank_k, num_pthreads); break;
            case 7:  res = ivf_search_simd        (q, vecdim, k, nprobe); break;
            case 8:  res = ivf_search_omp         (q, vecdim, k, nprobe, online_omp_threads); break;
            case 9:  res = ivf_search_pthread     (q, vecdim, k, nprobe, num_pthreads); break;
            ...
            case 12: res = ivf_pq_search_pthread  (q, vecdim, k, nprobe, rerank_k, num_pthreads); break;
            default:
    ...
    // 打印参数
    if (strategy == 2 || strategy == 6 || strategy == 9 || strategy == 12)
    ...
    if (strategy >= 7) free_ivf();
    if (strategy >= 10) free_ivf_pq();
    return 0;
}
