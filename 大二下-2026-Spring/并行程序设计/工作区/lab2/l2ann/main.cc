...
void offline_sq(float* base, size_t base_number, size_t vecdim) {
    ...
    sq_scale = 127.0f / max_val; // 对称量化，不使用偏移以保证 IP 排名正确
...
std::priority_queue<std::pair<float, uint32_t> > sq_search_simd(int8_t* base_i8, float* query, size_t base_number, size_t vecdim, size_t k) {
    std::priority_queue<std::pair<float, uint32_t> > q;
    int8_t q_i8[vecdim];
    for(size_t d = 0; d < vecdim; ++d) q_i8[d] = (int8_t)(query[d] * sq_scale);
...
// ==========================================
// 4. Strategy 4: PQ + Re-ranking SIMD
// 纯内积距离，通过重排提升召回率
// ==========================================
...
void offline_pq_ip(float* base, size_t base_number, size_t vecdim) {
    ...
    for(int m=0; m<PQ_M; ++m) {
        ...
        // 复用你原来的内积版 simple_kmeans (确保里面使用的是 calc_sub_dist)
        simple_kmeans(sub_data.data(), train_n, sub_dim, pq_centroids + m*PQ_K*sub_dim);
        
        #pragma omp parallel for // 这里强烈建议开启 OpenMP 提高离线速度[cite: 3]
        for(size_t i=0; i<base_number; ++i) {
            for(int m=0; m<PQ_M; ++m) {
                float max_ip = -1e9;
                int best = 0;
                ...
                for(int c=0; c<PQ_K; ++c) {
                    // 这里现在通过 SIMD 函数加速了子空间的内积计算[cite: 3]
...
std::priority_queue<std::pair<float, uint32_t> > pq_rerank_search_simd(
    ...
    // 3. 精排 (Re-ranking)：读取原向量计算精确 SIMD 内积
...
int main(int argc, char *argv[]) {
    ...
    int strategy = 4; // 对比不同算法：0 = Flat, 1 = Flat-SIMD, 2 = SQ-SIMD, 3 = PQ-SIMD
    ...
    for(size_t i = 0; i < test_number; ++i) {
        ...
        else if (strategy == 2) res = sq_search_simd(sq_base, test_query + i*vecdim, base_number, vecdim, k);
        else if (strategy == 3) res = pq_search_simd(test_query + i*vecdim, base_number, vecdim, k);
        else if (strategy == 4) res = pq_rerank_search_simd(test_query + i*vecdim, base, base_number, vecdim, k, 500); // 500 是 rerank_k，你可以调整它来观察 Latency 和 Recall 的权衡
